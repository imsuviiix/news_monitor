...
연합뉴스 · 뉴시스 · 뉴스1, 13개 언론사의 **사회면**에서 **전일 19:00 ~ 당일 07:00** 사이에 올라온
뉴스 헤드라인만 모아 1) 웹사이트에서 보여주고 2) 텔레그램으로 전송하는 프로젝트입니다.
**평일(월~금) 아침에만** 발행됩니다.
...
- 각 언론사는 네이버뉴스에 언론사 코드(`oid`)로 등록되어 있고, 네이버뉴스는
  ...
  전일 19:00~당일 07:00 구간에 해당하는 기사만 걸러냅니다.
- `collector.py`가 13개 언론사를 순회하며 수집한 결과를 `data/digest_YYYY-MM-DD.json`,
  `data/latest.json`으로 저장합니다.
- `app.py`(Flask)가 저장된 digest를 웹페이지로 보여줍니다.
- `telegram_bot.py`가 digest를 언론사별로 정리해 텔레그램으로 전송합니다.
- `run_daily.py`가 위 과정을 한 번에 실행하는 진입점이며, 평일 아침 7시 이후 cron/스케줄러로
  실행하는 것을 전제로 만들어졌습니다.

## 🚀 운영 구성: GitHub Actions + GitHub Pages (서버 불필요, 무료)

`.github/workflows/daily-digest.yml` 워크플로가 **평일 07:01(KST)** 에 자동으로:

1. 13개 언론사 야간 사회면 뉴스를 수집하고
2. 텔레그램으로 전송하고
...
13개사 모두 실수집 검증 완료.) 특정 언론사가 계속 0건이거나 FAIL이 뜨면 `config.py`의
...
```
1 7 * * 1-5 cd /path/to/news_monitor && /path/to/venv/bin/python run_daily.py >> run.log 2>&1
```
...
평일 07:01(KST)에 자동으로 `run_daily.py`와 동일한 작업을 수행합니다.

## 시간 구간 로직

"전일 19:00 ~ 당일 07:00"은 스크립트를 **실행하는 시점의 당일 날짜** 기준으로 계산됩니다
(`collector.get_window`). 예를 들어 7월 7일 07:01에 실행하면 7월 6일 19:00 ~ 7월 7일 07:00
구간의 기사를 수집합니다. 따라서 매일 07:00 이후(권장: 07:01)에 실행해야 그날의 창이
온전히 채워집니다.
...
.github/workflows/daily-digest.yml  평일 07:01 자동 수집+전송+배포
```
